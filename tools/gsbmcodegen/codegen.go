...
func Generate(ps *gsbmschema.PackageSet, schema *gsbmschema.Schema) ([]GeneratedFile, error) {
	...
	// Opaque struct children referenced by a generated parent must implement
	// ForgetPresenceTree and ForgetValuePresenceTree alongside the existing
	// MarshalGSBM/UnmarshalGSBM/Reset contract — emit.go calls these helpers
	// unconditionally on struct-typed fields. Without this precheck a missing
	// method only surfaces as a confusing `go build` error from generated code.
...
func emitFile(pkg *types.Package, named *types.Named, sd *gsbmschema.StructDecl) ([]byte, error) {
	...
	body.WriteString("\n")
	e.emitForgetPresenceTree(&body, named, str, sd)
	body.WriteString("\n")
	e.emitForgetValuePresenceTree(&body, named, str, sd)
...
// opaqueKey is the (PkgPath, Name) lookup key for the opaque-set map.
// TypeArgs are deliberately omitted: an opaque generic origin and its
// instantiation share the same identity for "needs the Forget* helpers"
// — Go's per-instantiation generic methods resolve at the call site.
func opaqueKey(pkgPath, name string) string {
	if pkgPath == "" {
		return name
	}
	return pkgPath + "." + name
...
// requireOpaqueForgetMethods walks every struct-typed field of str and, when
// the child is an opaque type, asserts that it implements the two tree-cleanup
// helpers that emit.go calls unconditionally. The check mirrors exactly the
// shapes emit.go recognizes (value, pointer pointee, slice element, map value).
func requireOpaqueForgetMethods(str *types.Struct, opaqueSet map[string]bool) error {
	for f := range str.Fields() {
		var child *types.Named
		...
		if !opaqueSet[opaqueKey(pkgPath, obj.Name())] {
			continue
		}
		if missing := missingForgetMethods(child); len(missing) > 0 {
			return fmt.Errorf("%s: opaque type %s used as a struct-typed field must implement %s — a body that calls gsbm.ForgetPresence(v) and stops is sufficient",
				f.Name(), opaqueKey(pkgPath, obj.Name()), strings.Join(missing, " and "))
		}
	}
	return nil
}

// missingForgetMethods returns the names of the cleanup helpers absent from
// the method set of *named. Both must be pointer-receiver methods with no
// parameters and no results to satisfy the call sites emit.go generates.
func missingForgetMethods(named *types.Named) []string {
	required := []string{"ForgetPresenceTree", "ForgetValuePresenceTree"}
	var missing []string
	mset := types.NewMethodSet(types.NewPointer(named))
	for _, name := range required {
		sel := mset.Lookup(named.Obj().Pkg(), name)
		if sel == nil || !isNullaryMethod(sel.Obj()) {
...
func isNullaryMethod(obj types.Object) bool {
