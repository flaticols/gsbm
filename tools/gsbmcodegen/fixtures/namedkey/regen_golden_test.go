...
// TestGoldenNamedKey asserts the committed <type>_gsbm.go in this fixture
// is byte-identical to what Generate produces from the live types.go.
func TestGoldenNamedKey(t *testing.T) {
	dir := fixtureDir(t)
...
// TestGoldenNamedKeyArena pins the arena-mode companion for the //gsbm:root
// Counts.
func TestGoldenNamedKeyArena(t *testing.T) {
	dir := fixtureDir(t)
	ps := loadFixture(t, dir)
	res := gsbmschema.Analyze(ps)
	if len(res.Issues) > 0 {
