...
// DecodeWideRecord decodes data into a *WideRecord allocated from a. The returned
...
// returned *WideRecord itself, plus any string/slice header it contains,
...
func DecodeWideRecord(data []byte, a *gsbmarena.Arena) (*WideRecord, error) {
	v := gsbmarena.AllocStruct[WideRecord](a)
	v.Reset()
...
// DecodeWideRecordBody is the headerless variant: data is the raw root body
// without the 12-byte blob header.
func DecodeWideRecordBody(data []byte, a *gsbmarena.Arena) (*WideRecord, error) {
	v := gsbmarena.AllocStruct[WideRecord](a)
	v.Reset()
	r := gsbm.NewReader(data)
	r.SetAllocator(a)
...
// DetachWideRecord returns a heap-allocated *WideRecord with no references into any
// arena. Implemented as a re-encode + heap decode round-trip so the
// heap-mode unmarshal body is the single source of truth.
func DetachWideRecord(o *WideRecord) (*WideRecord, error) {
