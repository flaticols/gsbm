...
// DecodeRecord decodes data into a *Record allocated from a. The returned
...
// returned *Record itself, plus any string/slice header it contains,
...
func DecodeRecord(data []byte, a *gsbmarena.Arena) (*Record, error) {
	v := gsbmarena.AllocStruct[Record](a)
	v.Reset()
...
// DecodeRecordBody is the headerless variant: data is the raw root body
// without the 12-byte blob header.
func DecodeRecordBody(data []byte, a *gsbmarena.Arena) (*Record, error) {
	v := gsbmarena.AllocStruct[Record](a)
	v.Reset()
	r := gsbm.NewReader(data)
	r.SetAllocator(a)
