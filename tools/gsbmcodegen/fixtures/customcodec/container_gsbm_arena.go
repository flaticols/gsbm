...
// DecodeContainer decodes data into a *Container allocated from a. The returned
...
// returned *Container itself, plus any string/slice header it contains,
...
func DecodeContainer(data []byte, a *gsbmarena.Arena) (*Container, error) {
	v := gsbmarena.AllocStruct[Container](a)
	v.Reset()
...
// DecodeContainerBody is the headerless variant: data is the raw root body
// without the 12-byte blob header.
func DecodeContainerBody(data []byte, a *gsbmarena.Arena) (*Container, error) {
	v := gsbmarena.AllocStruct[Container](a)
	v.Reset()
	r := gsbm.NewReader(data)
	r.SetAllocator(a)
...
// DetachContainer returns a heap-allocated *Container with no references into any
// arena. Implemented as a re-encode + heap decode round-trip so the
// heap-mode unmarshal body is the single source of truth.
func DetachContainer(o *Container) (*Container, error) {
