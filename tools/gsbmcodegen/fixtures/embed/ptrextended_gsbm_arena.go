...
// DecodePtrExtendedBody is the headerless variant: data is the raw root body
// without the 12-byte blob header.
func DecodePtrExtendedBody(data []byte, a *gsbmarena.Arena) (*PtrExtended, error) {
	v := gsbmarena.AllocStruct[PtrExtended](a)
	v.Reset()
	r := gsbm.NewReader(data)
	r.SetAllocator(a)
...
// DetachPtrExtended returns a heap-allocated *PtrExtended with no references into any
// arena. Implemented as a re-encode + heap decode round-trip so the
// heap-mode unmarshal body is the single source of truth.
func DetachPtrExtended(o *PtrExtended) (*PtrExtended, error) {
