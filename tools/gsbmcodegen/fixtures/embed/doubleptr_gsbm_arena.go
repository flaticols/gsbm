...
// DecodeDoublePtr decodes data into a *DoublePtr allocated from a. The returned
...
// returned *DoublePtr itself, plus any string/slice header it contains,
...
func DecodeDoublePtr(data []byte, a *gsbmarena.Arena) (*DoublePtr, error) {
	v := gsbmarena.AllocStruct[DoublePtr](a)
	v.Reset()
...
// DecodeDoublePtrBody is the headerless variant: data is the raw root body
// without the 12-byte blob header.
func DecodeDoublePtrBody(data []byte, a *gsbmarena.Arena) (*DoublePtr, error) {
	v := gsbmarena.AllocStruct[DoublePtr](a)
	v.Reset()
	r := gsbm.NewReader(data)
	r.SetAllocator(a)
...
// DetachDoublePtr returns a heap-allocated *DoublePtr with no references into any
// arena. Implemented as a re-encode + heap decode round-trip so the
// heap-mode unmarshal body is the single source of truth.
func DetachDoublePtr(o *DoublePtr) (*DoublePtr, error) {
