...
// TestLoadFromDirsTolerantOfStaleGeneratedFiles — a stale `_gsbm.go`
// companion referencing a renamed/removed user type must NOT block the
// loader. Otherwise `gsbmschema gen` (the very command that would
// regenerate the file) becomes unreachable after a refactor.
func TestLoadFromDirsTolerantOfStaleGeneratedFiles(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "go.mod"),
		[]byte("module example.com/stale\n\ngo 1.24\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(pkgDir, "types.go"), []byte(`package pkg
...
`), 0o644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(pkgDir, "thing_gsbm.go"), []byte(`// Code generated by gsbmcodegen. DO NOT EDIT.
package pkg

func (v *RemovedType) MarshalGSBM() {}
`), 0o644); err != nil {
		t.Fatal(err)
	}
	ps, err := LoadFromDirs([]string{pkgDir})
	if err != nil {
		t.Fatalf("LoadFromDirs: %v (stale _gsbm.go should not block load)", err)
	}
	if len(ps.Packages) != 1 {
		t.Fatalf("expected 1 package, got %d", len(ps.Packages))
	}
	for _, f := range ps.Packages[0].Files {
		name := filepath.Base(ps.Fset.Position(f.Pos()).Filename)
		if strings.HasSuffix(name, "_gsbm.go") {
			t.Fatalf("generated companion %s leaked into Files", name)
		}
	}
}

// TestLoadFromDirsSurfacesTransitiveDepErrors — when a transitive
// dependency fails to typecheck, the underlying file:line diagnostic
// must surface, not just a flattened "could not import X" on the
// consumer. Otherwise the user gets a misleading schema-level message
// instead of the root cause.
func TestLoadFromDirsSurfacesTransitiveDepErrors(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "go.mod"),
		[]byte("module example.com/transerr\n\ngo 1.24\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	apiDir := filepath.Join(root, "api")
	badDir := filepath.Join(root, "internal", "bad")
	for _, d := range []string{apiDir, badDir} {
		if err := os.MkdirAll(d, 0o755); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.WriteFile(filepath.Join(apiDir, "types.go"), []byte(`package api

import "example.com/transerr/internal/bad"
...
`), 0o644); err != nil {
	...
	_, err := LoadFromDirs([]string{apiDir})
	if err == nil {
		t.Fatal("expected load error from broken transitive dep, got nil")
	}
