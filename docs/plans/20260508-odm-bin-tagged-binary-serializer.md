...
- [x] Implement arena `Reader` taking `(data []byte, a *Arena)`; arena-mode decoders allocate slice backing arrays, the root struct, and string bytes via the arena (`odm.MakeSlice[T]` → per-T `*odm.TypedPool[T]` via `SlicePoolStore`; `odmarena.AllocStruct[T]` for the root; `arena.AcquireString(b)` for strings, `unsafe.String`-aliasing arena bytes). The heap-mode `*odm.Reader` is reused with the arena installed via `SetAllocator`. Generic interface methods do not exist in Go, so the seam is the typed pool map keyed on `reflect.Type`. Maps stay heap-allocated per Path A. Optional pointer fields (`*Nested`, `*int64`, `*Label`, …) and `[]byte` fields stay heap-allocated per **Path A1** — they are not on the seamed allocator path; lifting them onto the arena is tracked as a follow-up in docs/implementation.md §4.5.
...
- [x] Verify all requirements from Overview are implemented: tagged wire format end-to-end (`storage/odm` Writer/Reader + 8-byte header), codegen + schema-as-graph (`tools/odmschema` + `tools/odmcodegen`), Spanner dual-write/read-switch/cutover complete (skipped per Task 6/7/8 notes — those steps live in the consuming `ooms-offerengine` repo; gsbm owns the codec contract used by the deployment-side rollout), arena-mode shipped for read-heavy paths (`storage/odmarena` + `--mode=arena` codegen)
- [x] Verify the wire format invariants from Technical Details are enforced by tests: header validation (`TestHeaderRoundTrip`/`TestHeaderBadMagic`/`TestHeaderUnsupportedVer`/`TestHeaderTruncated`), unknown-tag skipping (`TestSkipUnknownField`/`TestSkipReservedWireType`/`TestUnknownTagSkipped`), presence-byte rules including reserved-state rejection (`TestPresenceReservedStateRejected`/`TestPresenceReservedUpperBitsRejected`/`TestPresenceZeroRejectedForNamed`/`TestPresenceEligibilityRule`), append-only classifier rules (`TestClassifySafeChanges`/`TestClassifyDeprecateAndResurrect`/`TestClassifyBreakingChanges`/`TestAllowBreakingOverride`/`TestRenamePreservesTag`/`TestComputeSchVerStable`), map-key constraints (`TestValidateMapKey` + codegen-side `ErrInvalidMapKey`), no-cycle validation (`TestValidateNoCycles`)
- [x] Verify benchmark targets met: encode covered by `TestEncodeWarmAllocsBoundedByPool` (0 allocs/op warm with pooled buffer on the sample fixture; absolute "1 alloc/op on BDD" target carried to the deployment cluster's local micro-benchmarks per the Context section — no BDD payload exists in this repo, consistent with the Task 1/Task 5 skip notes), decode `DecodeInto` + pool covered by `TestPoolWarmupConvergence` and `TestDecodeIntoReusesCapacity` (warm path lands at the per-string copy floor; the absolute "single-digit allocs/op on BDD" target ships with the consuming repo), arena decode covered by `TestArenaAllocsScaleSublinearly` (interpreted per advisor as "doesn't scale with N" rather than literal constancy — 4-item vs 64-item decode; the test exercises the slice-of-Item path which is on the arena allocator seam, so it does not flag the Path A1 pointer/`[]byte` heap-allocations documented in docs/implementation.md §4.5)
- [x] Verify rollback safety: `TestRollbackMissingTagsZeroDecode` exercises a hand-built `fmtVer=1` blob carrying only a subset of the schema's tags, decoding cleanly and zero-filling the missing fields; paired with `TestUnknownTagSkipped` (forward compat) this proves rollback in both directions
- [x] Verify cross-mode equivalence: `TestArenaCrossModeRoundTrip` (heap encode → arena decode → values match → Detach → DeepEqual to heap decode) and `FuzzArenaDecodeAgainstHeap` (5s local fuzz run produced no divergence between heap and arena decoders against the shared corpus; the fuzz path also forced the `Reader.ReadLength` oversized-count hardening recorded in Task 9)
- [x] run full project test suite (`go test ./...` and `go vet ./...` clean)
- [x] run project linter - all issues must be fixed (`golangci-lint run ./...` reports 0 issues; project has no `.golangci.yml`, so this uses the linter's default enabled set)
...
- **Path A1 → arena pointer/[]byte routing**: optional pointer fields (`*Nested`, `*int64`, `*Label`, …) and `[]byte` fields ship Path A1 — heap-allocated even in arena mode, because the heap-mode `UnmarshalODM` body emits direct `&T{}` / `&tmp` / `append(dst[:0], b...)` and those sites are not on the seamed allocator path. Lifting them onto the arena needs either a second emitter pass or new generic seam helpers; gated on whether they show up as a measurable hotspot after Path A1 ships. See docs/implementation.md §4.5.
