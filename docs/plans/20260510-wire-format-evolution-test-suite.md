...
type Catalog struct {
    ID       string             `bin:"1"`
    Sections []*Section         `bin:"2"`           // []*nullable
    Tags     map[string]*Tag    `bin:"3"`           // map of *nullable
    Hot      map[Code]*Tag      `bin:"4"`           // named-type map key
...
type Item struct {
    SKU   string  `bin:"1"`
    Note  *string `bin:"2"` // nullable inside slice element
}
...
type Code string // named-string for map key
...
Why this shape: the new types must collide with the encodings the explorer flagged as missing. `[]*Section` and `[]*Item` exercise the slice-of-nullable path; `map[string]*Tag` and `map[Code]*Tag` exercise map-of-nullable plus named map key. `Section → Item` is two-deep named composition, complementing the existing `DeepNested → Branch → Leaf` shape. `Tail EdgeMarker` at tag `536870911 = 2^29 - 1` pins the upper varint boundary for the field-key encoding (one more would overflow per spec §3.1).
...
- [ ] create `tools/gsbmcodegen/fixtures/graph/types.go` with the type set in Technical Details (`Catalog`, `Section`, `Item`, `Tag`, `Code`, `EdgeMarker`)
- [ ] create `tools/gsbmcodegen/fixtures/graph/regen_golden_test.go` mirroring `tools/gsbmcodegen/regen_golden_test.go` so `REGEN_GOLDEN=1` regenerates this package's goldens too
- [ ] generate goldens: `REGEN_GOLDEN=1 go test ./tools/gsbmcodegen/fixtures/graph/ -run TestRegenGolden`
- [ ] verify the generated `catalog_gsbm.go` compiles, includes `MarshalGSBM`/`UnmarshalGSBM`/`Reset`/`FieldPresent`, and emits the high-tag field-key without varint truncation
- [ ] add a smoke test in `graph_test.go` that round-trips an empty `Catalog{}` (asserts the package wires up before tackling complex shapes in Task 2)
- [ ] write tests for the smoke round-trip
