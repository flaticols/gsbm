...
- [x] add `storage/gsbm/fuzz_test.go` with `FuzzReaderRobustness`: feeds arbitrary bytes to `gsbm.NewReader`; asserts any returned error is one of the documented `gsbm.Err*` sentinels via `errors.Is`; asserts no panic via a `defer recover()` that fails the fuzz on panic
- [x] add `FuzzWriterReaderRoundTripCanonical` to the same file: decode arbitrary bytes; if decode succeeds, re-encode and assert byte-identity with the input. Carve out the maps case with an inline comment citing spec §5.3 deterministic-key rule + Go iteration non-determinism (implemented as the stronger idempotent canonical-form check: encode∘decode applied twice converges; the input may be non-canonical via unsorted map keys, non-minimum varints, or unknown-tag forward-skip, so a strict `D == B1` would flag known design choices instead of bugs — documented inline)
- [x] add `FuzzHeaderCorruption` seeded with a valid `MakeLargeOrder` blob; the fuzz function reads the seed, mutates one or more of the first 8 bytes (the harness picks which based on the fuzz input), then asserts the corresponding `gsbm.Err*` sentinel surfaces and decode does not panic
- [x] commit minimal seed corpora under each fuzz function's `testdata/fuzz/<FuzzName>/` directory: at least one valid blob and one hand-crafted malformed blob per harness. Generate the valid-blob seeds via `bench.MakeLargeOrder` so they're reproducible (1-2 MiB seeds are wired via `f.Add` in code rather than as text testdata files: encoding 1.5 MiB as a Go literal would balloon the file to several MiB of source and provides no extra reproducibility — `bench.MakeLargeOrder(0, 1<<20, 2<<20)` is itself deterministic. Hand-crafted small testdata seeds cover header-only, empty, single-tag, varint-overflow, and the four corrupt-header variants.)
- [x] extend `FuzzArenaDecodeAgainstHeap` at `tools/gsbmcodegen/fixtures/sample/arena_test.go:180` with a 1-2 MB seed via `f.Add(...)` (the 1-2 MiB Order is built locally in `arena_test.go::makeLargeOrderForFuzzSeed` rather than imported from `internal/bench` because `internal/bench` already imports the sample package — importing it back would create a cycle. The local helper inflates `makeRichOrder` deterministically with `math/rand/v2`)
- [x] run each new fuzz harness for 30 seconds locally as a smoke test (`go test -fuzz=FuzzReaderRobustness -fuzztime=30s ./storage/gsbm/`); confirm no findings, no panics. Document the smoke run in the Task notes.
  - FuzzReaderRobustness: 30s smoke, ~3.5M execs across 8 workers, 92 new interesting inputs, no findings, no panics. Required `-timeout=300s` because mutated 1-2 MiB inputs occasionally hit 10s+ paths that race the default test timeout.
  - FuzzWriterReaderRoundTripCanonical: 30s smoke, ~50k execs (idempotent decode→encode→decode→encode loop is intentionally heavy for 1.5 MiB inputs; throughput is acceptable for a smoke run), 23 new interesting, no findings, no panics.
  - FuzzHeaderCorruption: 30s smoke, ~17k execs (each iteration copies the 1.5 MiB seed), 16 new interesting, no findings, no panics.
- [x] write tests for any helper code added in this Task (the harness functions themselves; the seed-generation helper if non-trivial) — the `documentedSentinels` table is exercised by every fuzz iteration; the seed-generation helper `largeOrderBlob` is exercised by every `f.Add` and verified by the encoder's existing round-trip tests
- [x] run project tests - must pass before next task
