...
- Pointer embedding (`type Outer struct { *Inner; ... }`) is permitted and flattens the same way. On encode, if the embedded pointer is nil, all flattened fields are skipped (same wire bytes as a value embed whose fields were all zero-elided). On decode, the embedded struct is lazily allocated on the first incoming tag belonging to it; if no such tag arrives, the pointer stays nil. This matches the "tag not present" rule from §7.2.
