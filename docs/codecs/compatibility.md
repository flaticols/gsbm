...
Forward-compatible for any reader at the new schema: the decoder
accepts the full int64 range and reads any value the writer produced.
For a reader at the old schema (no `type=int64` knowledge — same
behavior as an un-annotated `int`), the field still decodes correctly
for values in `[MinInt32, MaxInt32]`, but values outside that range
surface `ErrIntegerOverflow` and the field is rejected. This is
graceful rejection, not silent corruption — old readers cannot
misinterpret a wider value, but they cannot read it either. Sequence
the rollout so all readers that must see out-of-range values have the
